...
class Network:
    ...
    def to_pyg(self):
        ...
        if self._type == "IAB":
            ...
            pad_value = -1
            def pad_to_max_length(values, max_length, pad_value=0):
                padded_values = [
                    value + [pad_value] * (max_length - len(value))
                    for value in values
                ]
                return padded_values

            # 각 노드의 UL과 DL 길이를 확인하고 패딩 적용
            ul_values_list = [value['UL'] for value in self._net_link_dict.values()]
            dl_values_list = [value['DL'] for value in self._net_link_dict.values()]

            max_length = max(max(len(ul), len(dl)) for ul, dl in zip(ul_values_list, dl_values_list)) # 4

            ul_values_padded = pad_to_max_length(ul_values_list, max_length, pad_value)
            dl_values_padded = pad_to_max_length(dl_values_list, max_length, pad_value)

            ul_values = torch.tensor(ul_values_padded, dtype=torch.int32)
            dl_values = torch.tensor(dl_values_padded, dtype=torch.int32)
            ...
            net_data = net_data[net_data[:,0] != -1]
            net_map = net_data

            # 각 노드의 원래 길이를 저장합니다
            # ul_lengths = [len(value['UL']) for value in self._net_link_dict.values()]
            # dl_lengths = [len(value['DL']) for value in self._net_link_dict.values()]
            # 패딩된 값을 나중에 제거하는 예시
            # def remove_padding(padded_values, original_lengths):
            #     unpadded_values = []
            #     for padded, length in zip(padded_values, original_lengths):
            #         unpadded_values.append(padded[:length])
            #     return unpadded_values

            # # 패딩된 값을 제거합니다
            # ul_values_unpadded = remove_padding(ul_values.tolist(), ul_lengths)
            # dl_values_unpadded = remove_padding(dl_values.tolist(), dl_lengths)
            ###########################################################################
...
class NetworkGenerator:
    ...
    def _deploy_combat_unit(self, G):
        ...
        while pending_unit_list:
            ...
            if level < 3:
                ...
                self._deploy_single_combat_unit(G, node_idx, level, max_comm_dist,  np.random.randint(2, max_sub_unit+1), comm_ability, pos_low,
                ...
                # type 1
                # random_angle_degree = np.array([np.random.uniform(0,120), np.random.uniform(120,240), np.random.uniform(240,360)])
                # random_angle_radian = np.deg2rad(random_angle_degree)
                # type 2
                # random_angle_radian = np.random.uniform(0,2*np.pi, 3)
                # type 3
                # random_angle_radian = np.array([np.random.uniform(0,(2/3)*np.pi), np.random.uniform((2/3)*np.pi, (4/3)*np.pi), np.random.uniform((4/3)*np.pi, 2*np.pi)])
                range_ = self._config['range'][level]
                parent_child_link = G.nodes[parent_idx]['n_child_link']


                # angle_ranges = [(0, 2/3*np.pi),
                #             (2/3*np.pi, 4/3*np.pi),
                #             (4/3*np.pi, 2*np.pi)]
                
                # if parent_child_link < parent_max_sub_unit and parent_child_link < 3:
                #     random_angle_radian = np.random.uniform(angle_ranges[parent_child_link][0],angle_ranges[parent_child_link][1])
                # elif parent_child_link < parent_max_sub_unit and parent_child_link >= 3:
                #     random = np.random.randint(1, 4) - 1 # Randomly add a number from 0 to 2
                #     random_angle_radian = np.random.uniform(angle_ranges[random][0],angle_ranges[random][1])
                # else:
                #     pass
                
                angle_ranges = []
                for i in range(parent_max_sub_unit):
                    
                    angle_ranges.append((2*np.pi*((i-0.5)/parent_max_sub_unit), 2*np.pi*((i+0.5)/parent_max_sub_unit)))
                
                if parent_child_link < parent_max_sub_unit:
                    random_angle_radian = np.random.uniform(angle_ranges[parent_child_link][0],angle_ranges[parent_child_link][1])
                    


                cos_value = np.cos(random_angle_radian)
                sin_value = np.sin(random_angle_radian)
 
                x_ = np.random.uniform(0,1)
                random_range = np.sqrt(x_) * range_
 
                pos_add = np.array([cos_value*random_range, sin_value*random_range])
 
                pos = parent_pos + pos_add
                if max_sub_unit != 0:
                    max_sub_unit= np.random.randint(2, max_sub_unit+1)
    ...
    def _show_network_graph(self, G):
        ...
        labels = {node: G.nodes[node]['level'] for node in G.nodes}
        nx.draw_networkx_nodes(G, pos = pos, node_color=[self._config['color_map'][G.nodes[node]['level']] for node in G.nodes()], node_size=50)
        nx.draw_networkx_edges(G, pos = pos)
        nx.draw_networkx_labels(G, pos, labels, font_size=8, font_color='black')
        plt.axis('on')
        plt.show()
        
    def _show_network_graph_colored(self, G, net_link_dict):
    ...
    def get_interference_graph_colored(self, nG):  # nG: network graph
...
if __name__ == '__main__':
    ...
    # iGt,iGa,net_link_dict = ng.get_interference_graph(nG)
    # nodd = ng.get_link_color()
    # ng._show_network_graph_colored(nG, net_link_dict)
    ...
    # Test save dataset
    ng.generate_dataset(num_network=100, file_name_prefix='wcnc_test_37')
